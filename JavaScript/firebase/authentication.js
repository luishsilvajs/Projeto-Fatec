...
function loginFirebase(email, senha){
    ...
    .then(result => {
        console.log(result.user)  //aparece em preto na tela
        window.location.href = `${urlApp}/menu.html`
    })
    .catch(error => {
        console.error(error.code)    //aparece em vermelho na tela
        alerta(`Erro: não foi possível efetuar o login - ${errors[error.code]}`,
            'danger')
    })
}
...
function loginGoogle(){
    ...
    .then((result) => {
        ...
        alerta(`Erro ao efetuar Login: ${errors[error.code]}`, 'danger')
    })
